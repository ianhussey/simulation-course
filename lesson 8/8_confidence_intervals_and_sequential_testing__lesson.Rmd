...
TODO ADD DANCE OF THE P VALUES
...
Note that I have chosen values for `set.seed` that produce particularly extreme results that dance back and forth. Try changing the seed values and the parameter estimates. E.g.:

- What seeds produce very different patterns of results (e.g., `2837`).
- Make the effect size quite large. How quickly do results stabilize relative to when the sample size is small?
- How wide are 95% CIs when the sample size is much larger, e.g. 1000? (you can also change the `minimum_n_per_group` and `additional_n_per_group_per_step` values to be larger to make the plots more readable)
