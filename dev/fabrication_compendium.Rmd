...
write_rds(simulation, "simulation_unwarranted_data_exclusion.rds", compress = "gz")
...
simulation <- read_rds("simulation_unwarranted_data_exclusion.rds")

iteration <- 22

left_join(
  simulation$generated_data[[iteration]],
  simulation$altered_data[[iteration]]
) |>
...
bind_cols(
  simulation$analysis_results_original_data[[iteration]] |>
    rename(p_original = p),
  simulation$analysis_results_altered_data[[iteration]] |>
    rename(p_altered = p),
) |>
...
write_rds(simulation, "simulation_condition_switching.rds", compress = "gz")
...
simulation <- read_rds("simulation_condition_switching.rds")

iteration <- 23

ggplot(simulation$altered_data[[iteration]], aes(condition, score, color = altered)) +
  geom_point(position = position_jitter(width = 0.1)) +
  scale_color_viridis_d(begin = 0.3, end = 0.7) +
  theme_linedraw()
