...
It's easier to see from this plot that the therapy does indeed work. The values are also realistic: the means, SDs, and effect sizes are reasonable for BDI-II scores for an effective therapy. 

[TODO] say more about the purpose of simulation, to provide ground truth. The relation and direction between sample and population in evaluating results (sample -> population) vs in simulations (population -> sample). The utility of knowing what the real answer is.

[TODO] we are taught "violating statistical assumptions is bad", but how bad? in what contexts, and why? What are the specific consequences of doing so? Simulation lets you answer this. This course will teach you to run simulation studies, including the concepts and the implementations in R.
